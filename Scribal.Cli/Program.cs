...
await RunMainLoop();
static async Task DisplayWelcome()
{
    AnsiConsole.Clear();
...
async Task RunMainLoop()
{
    ...
    while (isRunning)
    {
        ...
        if (input.StartsWith("/"))
        {
            ...
            var arguments = input.Length > commandText.Length ? input[(commandText.Length + 1)..].Trim() : string.Empty;
...
static async Task<bool> QuitCommand(string arguments)
{
...
static async Task<bool> HelpCommand(string arguments)
{
...
static async Task<bool> NewProjectCommand(string arguments)
{
...
static async Task<bool> OpenProjectCommand(string arguments)
{
...
static async Task<bool> SaveCommand(string arguments)
{
...
static async Task<bool> ListCharactersCommand(string arguments)
{
    ...
    var table = new Table().Border(TableBorder.Rounded).AddColumn("Name").AddColumn("Role").AddColumn("Description");
...
static async Task<bool> ViewPlotCommand(string arguments)
{
