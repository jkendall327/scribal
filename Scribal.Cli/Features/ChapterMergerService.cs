...
public class ChapterMergerService : IChapterMergerService
{
    ...
    public async Task<bool> MergeChapterAsync(ChapterState sourceChapter, CancellationToken cancellationToken)
    {
        ...
        if (!targetableChapters.Any())
        {
            ...
            _logger.LogInformation("No target chapters available for merging with source chapter {SourceChapterNumber}", sourceChapter.Number);
            return false;
        }
        ...
        _logger.LogInformation("Source Chapter: {SourceChapterNumber} ('{SourceChapterTitle}'). Target Chapter: {TargetChapterNumber} ('{TargetChapterTitle}')",
            sourceChapter.Number, sourceChapter.Title, selectedTargetChapterState.Number, selectedTargetChapterState.Title);

        _console.MarkupLine($"Merging [yellow]'{Markup.Escape(sourceChapter.Title)}'[/] into [green]'{Markup.Escape(selectedTargetChapterState.Title)}'[/].");
        ...
        var summaryPrompt = new SelectionPrompt<SummaryChoice>()
            ...
            .AddChoices(
                ...
                SummaryChoice.EnterNew
            )
            .UseConverter(choice => choice switch
            {
                SummaryChoice.KeepTarget => $"Keep target's current summary ('{Markup.Escape(selectedTargetChapterState.Summary ?? "empty")}')",
                SummaryChoice.UseSource => $"Use source's summary ('{Markup.Escape(sourceChapter.Summary ?? "empty")}')",
        ...
        switch (summaryDecision)
        {
            ...
            case SummaryChoice.EnterNew:
                newTargetSummary = _console.Ask<string>($"Enter the new summary for the merged chapter '{Markup.Escape(selectedTargetChapterState.Title)}':").Trim();
                if (cancellationToken.IsCancellationRequested) return false;
                break;
            default:
        ...
        var confirmMessage = $"Confirm merge: Chapter {sourceChapter.Number} ('{Markup.Escape(sourceChapter.Title)}') " +
                             $"will be merged into Chapter {selectedTargetChapterState.Number} ('{Markup.Escape(selectedTargetChapterState.Title)}'). " +
                             $"The source chapter will be deleted. The target chapter's summary will be '{Markup.Escape(newTargetSummary)}'. Proceed?";

        var confirmed = await _userInteraction.ConfirmAsync(confirmMessage);
        if (!confirmed || cancellationToken.IsCancellationRequested)
        {
            _logger.LogInformation("User cancelled chapter merge operation between source {SourceChapterNumber} and target {TargetChapterNumber}",
                sourceChapter.Number, selectedTargetChapterState.Number);
        ...
        _logger.LogInformation(
            ...
            sourceChapter.Number, selectedTargetChapterState.Number, newTargetSummary);

        var success = await _workspaceManager.MergeChaptersAsync(
            sourceChapter.Number,
        ...
        if (success)
        {
            _logger.LogInformation(
                ...
                sourceChapter.Number, selectedTargetChapterState.Number);
            _console.MarkupLine($"[bold green]Successfully merged '{Markup.Escape(sourceChapter.Title)}' into '{Markup.Escape(selectedTargetChapterState.Title)}'.[/]");
        }
        else
        {
            _logger.LogError(
